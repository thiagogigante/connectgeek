...
const startAutoScroll=()=>{stopAutoScroll();if(!matchMedia('(prefers-reduced-motion: reduce)').matches)autoScroll=setInterval(()=>{const atEnd=galleryTrack.scrollLeft+galleryTrack.clientWidth>=galleryTrack.scrollWidth-4;galleryTrack.scrollTo({left:atEnd?0:galleryTrack.scrollLeft+224,behavior:'smooth'})},3800)};
